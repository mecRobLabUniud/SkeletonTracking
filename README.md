...
│   ├── COLLcheck.hpp, examples_common.hpp, utils.hpp
│   └── .temp/                  # Unused / abandoned code (not compiled)
├── src/
...
│   ├── robot_control/          # Old Franka SDK utilities (not built)
│   └── .temp/                  # Unused / abandoned code
└── scripts/
    ...
    ├── training.py             # YOLO fine-tuning helper
...
- ZeroMQ (`libzmq`)
- libfranka — note that `CMakeLists.txt` currently declares `find_package(Franka 0.8.0 REQUIRED)` even though the active executables do not use it, so a libfranka build is still needed for CMake to configure.
...
- Some folders (`include/.temp/`, `src/.temp/`, `src/robot_control/`) contain experimental or legacy code that is **not compiled** by default.
